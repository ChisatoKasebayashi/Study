...
d_ball = np.array(data.iloc[:,0:2])
d_goal = np.array(data.iloc[:,2:4])
l_ball = np.array(data.iloc[:,4:6])
l_pole0 = np.array(data.iloc[:,6:8])
l_pole1 = np.array(data.iloc[:,8:10])
g_ball = np.array(data.iloc[:,10:12])
g_pole0 = np.array(data.iloc[:,12:14])
g_pole1 = np.array(data.iloc[:,14:16])
g_robot = np.array(data.iloc[:,16:19])
ljoy = np.array(data.iloc[:,19:21])
rjoy = np.array(data.iloc[:,21:23])
...
def calcObjectPosition(ball, robot, pole0, pole1, im_w, im_h,step):
    print(ball)
    print(ball[0])
    print(ball[1])
    bx, by = convertPoint(ball[0],ball[1],im_w, im_h)
    rx, ry = convertPoint(robot[0],robot[1],im_w, im_h)
    p0x, p0y = convertPoint(pole0[0],pole0[1],im_w, im_h)
    p1x, p1y = convertPoint(pole1[0],pole1[1],im_w, im_h)
    return [bx, by, rx, ry, p0x, p0y, p1x, p1y]
...
#        lbx, lby =  convertPoint(robot[step][0]+l_ball[step][0],robot[step][1]+l_ball[step][1],im_w, im_h)
        ...
        #print(g_ball[step])
        bx, by, rx, ry, p0x, p0y, p1x, p1y = calcObjectPosition(g_ball[step],g_robot[step],g_pole0[step],g_pole1[step],im_w,im_h,step)
