...
class MyCVAE(chainer.Chain):
    ...
    def encode(self, x, y):
        h0 = F.relu(self.le0(x))
        h1 = F.tanh(self.le1(h0))
        y1 = F.relu(self.le1_y(y))
        h2 = F.tanh(self.le2_y(y1))
        mu = self.le2_mu(F.concat([h1, h2]))
        ln_var = self.le2_ln_var(F.concat([h1, h2]))  # log(sigma**2)
        return mu, ln_var

    def decode(self, z, y, sigmoid=True):
        h0 = F.relu(self.ld0(z))
        h1 = F.tanh(self.ld1(h0))
        y1 = F.relu(self.ld1_y(y))
        h2 = F.tanh(self.ld2_y(y1))
